...
import { insertUserSchema, insertQuizSchema, insertQuestionSchema, insertQuizAttemptSchema, questionAnswerSchema, quizzes, questions } from "../shared/schema.js";
import { z } from "zod";
    ...
    app.get("/api/quizzes/slug/:urlSlug", async (req, res) => {
        try {
            ...
            if (!quiz) {
                ...
                const slugMatch = allQuizzesList.find(q => q.urlSlug?.toLowerCase() === urlSlug.toLowerCase());
                if (slugMatch) {
                    quiz = slugMatch;
