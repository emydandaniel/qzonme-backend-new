...
// Quiz routes
router.post("/api/quizzes", asyncHandler(async (req, res) => {
    const quizData = insertQuizSchema.parse(req.body);
    // Check for duplicate accessCode or urlSlug
    const existingQuiz = await db
        .select()
        .from(quizzes)
        .where(or(eq(quizzes.accessCode, quizData.accessCode), eq(quizzes.urlSlug, quizData.urlSlug)))
        .limit(1);
...
router.get("/api/quizzes/code/:accessCode", asyncHandler(async (req, res) => {
...
router.get("/api/quizzes/slug/:urlSlug", asyncHandler(async (req, res) => {
...
router.get("/api/quizzes/dashboard/:token", asyncHandler(async (req, res) => {
...
// Get quiz by ID
router.get("/api/quizzes/:id", asyncHandler(async (req, res) => {
    const { id } = req.params;
    const [quiz] = await db
        .select()
        .from(quizzes)
...
// Create question
router.post("/api/questions", asyncHandler(async (req, res) => {
...
router.get("/api/quizzes/:quizId/questions", asyncHandler(async (req, res) => {
...
// Create quiz attempt
router.post("/api/quiz-attempts", asyncHandler(async (req, res) => {
...
router.get("/api/quizzes/:quizId/attempts", asyncHandler(async (req, res) => {
    const { quizId } = req.params;
...
router.post("/api/upload-image", upload.single('image'), asyncHandler(async (req, res) => {
    if (!req.file) {
